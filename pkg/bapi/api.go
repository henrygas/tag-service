...
func (a *API) getAccessToken(c context.Context) (string, error) {
	body, err := a.httpGet(c, fmt.Sprintf("%s?app_key=%s&app_secret=%s",
...
func (a *API) GetTagList(c context.Context, name string) ([]byte, error) {
	token, err := a.getAccessToken(c)
	if err != nil {
		return nil, err
	}

	body, err := a.httpGet(c, fmt.Sprintf("%s?token=%s&name=%s",
